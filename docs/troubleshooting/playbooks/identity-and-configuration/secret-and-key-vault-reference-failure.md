...
| `az containerapp exec --name $APP_NAME --resource-group $RG --command "nslookup login.microsoftonline.com"` | Executes `nslookup` from inside a running replica to resolve the Entra authority host from the **workload replica's data-plane DNS view**. If the returned address is an RFC1918 private IP (or any non-Microsoft address), a Private DNS zone is overriding the public authority record on that VNet. Paired with the H0 success / H1 failure / H2 recovery pattern, this **strongly suggests** the ACA control-plane secret resolver was affected by a compatible DNS override and the OIDC discovery connection timed out before reaching the firewall (H4e) — but a replica `nslookup` alone does not prove the control-plane resolver saw an identical DNS view, because Azure does not document control-plane/data-plane DNS-view equivalence. |
...
| `az network nsg rule list --nsg-name $NSG_NAME --resource-group $NSG_RG` (after `az network vnet subnet show --ids $ACA_INFRA_SUBNET_ID --query "networkSecurityGroup.id" --output tsv` to locate the NSG) | Enumerates the outbound rules on the NSG attached to the ACA workload subnet. If an outbound rule denies 443/tcp to the `AzureActiveDirectory` service tag, to Microsoft-published Entra IP ranges, or to `Internet` with higher priority than any Allow rule, an NSG deny is by itself sufficient to explain the OIDC discovery failure and the zero-row H4 KQL result (H4c). When Azure Firewall is also present in the path, empty AzFW logs alone do not prove the NSG acted before the firewall — the NSG deny is the confirmed cause; the relative ordering of NSG versus firewall enforcement is not established by this evidence. `az network watcher flow-log show` only returns the flow-log *configuration* for a given NSG; if flow logs are enabled and you need to see the actual Deny records, query the exported flow-log data in the target storage account or use Traffic Analytics in the linked Log Analytics workspace. |
...
4. **H4c** — NSG rule enumeration. One CLI call pair (`az network vnet subnet show` → `az network nsg rule list`) is enough to rule in or out the documented `AzureActiveDirectory:443` deny pattern; flow logs are optional corroboration only.
...
The H4b (logging-gap) variant now has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — Logging Gap Variant (H4b) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4b.md), which proves that re-enabling the `AzureFirewallApplicationRule` diagnostic category restores observability of the denial without restoring connectivity. The H4e (custom DNS override) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — DNS Override Variant (H4e) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4e.md), which proves that a custom Private DNS zone overriding the Entra authority record breaks OIDC discovery before egress reaches the firewall, and that removing the override restores connectivity. The H4d (Virtual WAN + Routing Intent) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — Virtual WAN Routing Intent Variant (H4d) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4d.md), which proves that enabling Routing Intent to route the ACA infrastructure VNet's egress through the Virtual WAN Hub firewall breaks Entra authority reachability, and that disabling Routing Intent — with the firewall policy left unchanged — restores connectivity. The H4c (NSG deny) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — NSG Deny Variant (H4c) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4c.md), which proves that an outbound NSG rule denying AzureActiveDirectory:443 on the ACA workload subnet breaks Entra authority reachability before any firewall path, and that adding the documented higher-priority Allow restores connectivity. The H4g (Azure Firewall Premium TLS inspection) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — Firewall Premium TLS Inspection Variant (H4g) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4g.md), which proves that enabling TLS inspection on the Entra authority FQDNs breaks the control-plane's OIDC discovery call, and that exempting those FQDNs from TLS inspection — with all other topology unchanged — restores connectivity. The H4f (3rd-party NVA) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — Third-Party NVA Surrogate Variant (H4f) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4f.md), which substitutes a Linux forwarding VM for a vendor NVA and proves that a single forwarding-plane DROP rule for the `AzureActiveDirectory` service-tag prefixes on tcp/443 breaks Entra authority reachability — with no Azure Firewall and no TLS inspection anywhere in the topology — and that removing the same rule restores connectivity. With H4f complete, all six H4 network-path variants (H4b, H4c, H4d, H4e, H4f, H4g) now have end-to-end reproducer labs; the shared tracking issue is [issue #307](https://github.com/yeongseon/azure-container-apps-practical-guide/issues/307).
