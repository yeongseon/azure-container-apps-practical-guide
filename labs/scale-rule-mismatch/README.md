...
└── evidence/             # Captured CLI evidence
    ...
    ├── 11-containerapp-update-fix.json               # Phase 11: az containerapp update applying the fix (concurrentRequests=10, maxReplicas=10) — creates new revision (note: the `concurrentRequests` field is reported as `""` blank by the `--query` projection on the freshly-created new revision because the scale rule was not yet fully resolved on the immediate post-update response; see 12 for the post-fix `=10` confirmation)
    ...
    ├── 18-system-logs-scale-events-after-fix.json    # Phase 17: az monitor log-analytics query for ContainerAppSystemLogs_CL scale events in the post-fix load window (strict filter: `Reason_s startswith "Scal" or "KEDA"`; may return 0 rows when the platform emits only replica-lifecycle events like `AssigningReplica`)
    ├── 19-h2-gate.json                               # Phase 18: parsed H2 gate classification (`scale_rule_fixed_replicas_scaled_events_observed` when strict KQL gate fires, or `scale_rule_fixed_replicas_scaled_no_events` partial-pass when only replica count fires)
    ...
    └── 23-deployment-outputs.json                    # Post-run: `properties.outputs` of the Bicep deployment (six outputs: containerAppName, containerAppUrl, containerRegistryName, containerRegistryLoginServer, environmentName, logAnalyticsWorkspaceName — the LAW customer ID is queried separately via `az monitor log-analytics workspace show --query customerId`)
```
...
./verify.sh  2>&1 | tee evidence/00-verify-run.txt    # Apply fix, generate identical load against post-fix revision, capture replica + KQL evidence (the 2026-06-22 reproduction observed gate scale_rule_fixed_replicas_scaled_no_events partial-pass; the stricter scale_rule_fixed_replicas_scaled_events_observed gate fires only when the strict KEDA-attribution KQL filter returns >=1 row, which depends on whether the platform emits named scaler decisions vs only replica-lifecycle events during the post-fix load window)
...
- `verify.sh` Phase 17 queries `ContainerAppSystemLogs_CL` for the strict post-fix load window filtered to `Reason_s startswith "Scal" or "KEDA"` for the post-fix revision. The expected row count is `>= 1` for a strict pass, but it may return 0 rows when the platform emits only replica-lifecycle events (`AssigningReplica`, `PullingImage`, `PulledImage`) that are intentionally filtered out. In that case the replica count alone is the controlling signal and the H2 gate is a partial pass (`scale_rule_fixed_replicas_scaled_no_events`). This is the KEDA-attribution leg of the H2 gate.
- `verify.sh` Phase 18 emits the H2 gate JSON with one of four `gate_classification` values:
    - `scale_rule_fixed_replicas_scaled_events_observed` — all three sub-gates pass: post-fix revision Healthy AND max-during-load >= 2 AND scale events >= 1 (the stricter variant; the 2026-06-22 reproduction did NOT hit this gate because the platform emitted only replica-lifecycle events during the post-fix load window, so the strict KEDA-attribution KQL filter returned 0 rows and the partial-pass gate fired instead)
...
- Post-fix load window: up to 6 replicas (observed max in 2026-06-22 reproduction) running for 90 s under load; each replica is `0.5 vCPU`, `1 Gi` memory. At Consumption pricing this is well under USD $0.01 for the 90 s burst.
...
This lab observed (2026-06-22 reproduction in `koreacentral`) that when an HTTP scale rule is configured with a `concurrentRequests` threshold (500) far above realistic in-flight concurrency (60), the Azure Container Apps platform produces a deterministic, observation-only failure signature: the replica count stays pinned at `minReplicas` throughout the load window (1 / 1 / 1 / 1 at +15 s / +30 s / +60 s / +90 s), no KEDA scale-up events are recorded in `ContainerAppSystemLogs_CL`, and operators see degrading per-request latency without any platform error or warning. Microsoft Learn documents that `concurrentRequests` is the average target across all replicas ([Set scaling rules in Azure Container Apps](https://learn.microsoft.com/en-us/azure/container-apps/scale-app)); the consequence of setting it far above observed concurrency is what this lab demonstrates. The fix is to update the scale rule with a threshold proportional to observed in-flight concurrency and raise `maxReplicas` to allow horizontal scale-out: `az containerapp update --scale-rule-name http-rule --scale-rule-type http --scale-rule-metadata concurrentRequests=10 --max-replicas 10`. After the fix, KEDA scales the post-fix revision to 6 replicas under the identical 60-concurrent / 90 s load (post-fix revision Healthy 14 s after health polling began in `verify.sh` Phase 12), and `ContainerAppSystemLogs_CL` returns zero rows for the strict scaling-events filter (`Reason_s startswith "Scal" or "KEDA"`). The strict filter is intentionally narrow to prevent cross-revision contamination from benign teardown noise (`ScaledObjectCheckFailed` / `FailedGetScale`), so it excludes replica-lifecycle events (such as `AssigningReplica`) that the platform typically emits during scale-out — verify by re-running the query with `Reason_s contains "Replica"` against the same load window. The replica count (1 → 6) is the controlling signal for H2 in this case, not the KQL row count. [Inferred from KEDA HTTP add-on documented behavior; the strict filter's empty result does not by itself prove which event names the platform emitted during the post-fix load window.] The cheapest live-incident diagnostic for this failure mode is the combination of (1) the replica count from `az containerapp replica list --query "length(@)"` under sustained load (expected to be > 1 if scaling is working) and (2) a tight-window KQL query against `ContainerAppSystemLogs_CL` filtered to `RevisionName_s == <active-revision>` AND `Reason_s startswith "Scal"` (which may or may not return rows depending on whether KEDA emits named scaler decisions); the replica count alone is the controlling signal, and the KQL is corroborative. The matching playbook is [`docs/troubleshooting/playbooks/scaling-and-runtime/http-scaling-not-triggering.md`](../../docs/troubleshooting/playbooks/scaling-and-runtime/http-scaling-not-triggering.md); reproduce locally with `./trigger.sh` and `./verify.sh` to validate it against your own environment before training on-call engineers.
