...
// -----------------------------------------------------------------------------
// aca-secret-kv-ref-mi-network-path-h4c — infrastructure
// -----------------------------------------------------------------------------
//
// Reproduces the H4c variant for the customer failure:
...
//
// H4c deliberately removes Azure Firewall, UDR, and custom VNet DNS servers.
// The ONLY controlled variable is an outbound NSG rule on the ACA workload
// subnet that denies 443/TCP to the AzureActiveDirectory service tag.
//
// Phase behavior:
//   H0 baseline: NSG attached, no custom AAD deny/allow rule -> secret set succeeds.
//   H1 trigger:  Outbound NSG deny rule for AzureActiveDirectory:443 -> secret set fails.
//   H2 fix:      Higher-priority outbound NSG allow rule for AzureActiveDirectory:443
//                added while the deny remains -> secret set succeeds again.
//
...
// No custom VNet DNS servers. Azure-provided DNS stays in place so the H1/H2
// variable is only the outbound NSG rule set on the delegated ACA subnet.
