...
The H4b (logging-gap) variant now has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — Logging Gap Variant (H4b) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4b.md), which proves that re-enabling the `AzureFirewallApplicationRule` diagnostic category restores observability of the denial without restoring connectivity. The H4e (custom DNS override) variant also has a fully reproducible end-to-end lab — see the [ACA Secret Key Vault Reference — DNS Override Variant (H4e) Lab](../../lab-guides/aca-secret-kv-ref-mi-network-path-h4e.md), which proves that a custom Private DNS zone overriding the Entra authority record breaks OIDC discovery before egress reaches the firewall, and that removing the override restores connectivity. Follow-up reproducer labs for the remaining variants (H4c, H4d, H4f, H4g) are tracked in [issue #307](https://github.com/yeongseon/azure-container-apps-practical-guide/issues/307).
