...
|---|---|---:|---|---|---|
| `30-cohort-integrity-gate.json` | `evidence_cohort_is_internally_consistent_and_uncontaminated` | 4 | All 24 canonical files + cohort directory listing + 4 emitted Phase B gate JSONs + `evidence/README.md` | PASS | Cohort integrity gate. Confirms (a) all 24 canonical files exist on disk; (b) temporal coherence — the span from `01-deployment-result.timestamp` (`2026-06-22T13:20:46Z`) through `12-curl-after-fix.utc_completed` (`2026-06-22T13:28:23Z`) is 356 s, well within both the 30-minute strong window and the 60-minute fallback window; (c) no unexpected extras — only the 24 canonical files plus the four Phase B gate JSONs and `README.md` are present (no stale `.tmp`, `.bak`, `.swp` files, no captures from a different reproduction date); (d) `evidence/README.md` exists and references all four gate filenames so a reviewer can locate every emitted output. |
| `31-failure-attribution-gate.json` | `deployment_level_failure_attribution_to_manifest_unknown_in_failed_deployment_cohort` | 5 | `01-deployment-result.json` + `01-deployment-operations-failed.json` + `02-containerapp-show-baseline.json` + `03-revisions-list-baseline.json` + `04-acr-repository-list-baseline.json` | PASS | Failure attribution gate. Confirms (a) the deployment failed with the smoking-gun string `MANIFEST_UNKNOWN: manifest tagged by "does-not-exist" is not found` in `properties.statusMessage.error.message` of the failed deployment operation, scoped strictly to the failed-deployment cohort (a `cohort_binding_note` field is embedded in the gate JSON to record this scope); (b) the Container App resource itself is in `provisioningState=Failed`; (c) `latestRevisionName` is `null`; (d) `az containerapp revision list` returns an empty array; (e) `az acr repository list` does not include `labacr`. All five absence predicates are bound to the failed-deployment cohort, not to the post-fix state — Oracle's Q1 strategy verdict required this explicit cohort binding to prevent the "no-revision proves no-MANIFEST_UNKNOWN" inversion fallacy. |
| `32-recovery-materialization-gate.json` | `post_fix_recovery_materialized_healthy_revision_serving_traffic` | 4 | `08-containerapp-update-result.json` + `09-containerapp-show-after-fix.json` + `10-revisions-list-after-fix.json` + `11-acr-repository-list-after-fix.json` + `11-acr-repository-show-tags-after-fix.json` + `12-curl-after-fix.json` | PASS | Recovery materialization gate. Confirms (a) `az containerapp update --image <acr>/labacr:v1` succeeded and a `latestRevisionName` is populated post-fix; (b) the post-fix revision list contains at least one revision with `healthState=Healthy` and `trafficWeight=100`; (c) curl probe success rate is ≥ 8/10 (Strong path: ≥ 9/10; Fallback path: ≥ 8/10 — the only sub-gate where dual-path scaffolding is used per Oracle's Q2 verdict, both paths PASS for the observed 10/10); (d) the `labacr` repository and the `v1` tag are present in ACR post-fix. |
| `33-in-place-recovery-continuity-gate.json` | `recovery_is_in_place_on_same_app_identity_not_a_re_provision` | 3 | `02-containerapp-show-baseline.json` + `09-containerapp-show-after-fix.json` + Bicep declared identity | PASS | In-place recovery continuity gate. Confirms the recovery was an **in-place update** of the original Container App, not a tear-down-and-re-provision. (a) Same app identity: the Container App `id` and `name` match across baseline and post-fix captures (`ca-labacr-rnnljl` in resource group `rg-aca-lab-acr`); (b) State transition: `provisioningState` transitioned from `Failed` (baseline) to `Succeeded` (post-fix) without an intermediate Container App resource being created; (c) Image tag delta on the same registry: `template.containers[0].image` changed from `<acr>/labacr:does-not-exist` (baseline) to `<acr>/labacr:v1` (post-fix), with `<acr>` identical across both captures (`acrlabacrrnnljl.azurecr.io`). |

The four gates together block four classes of overclaim: **evidence-pack-was-contaminated-with-stale-or-foreign-artifacts** is blocked by Gate 30's foreign-artifact check + temporal coherence window; **the-smoking-gun-could-have-come-from-a-different-deployment-operation** is blocked by Gate 31's cohort-binding of all five sub-predicates to the failed-deployment operations only; **recovery-was-partial-or-curl-rate-was-insufficient** is blocked by Gate 32's BOTH-not-OR sub-gate composition (all four sub-gates must independently pass; sub-c is the only sub-gate where Strong AND Fallback path are both evaluated, per Oracle's Q2 single-canonical-source verdict); and **the-recovery-was-a-tear-down-and-re-provision-not-an-in-place-fix** is blocked by Gate 33's identity + state-transition + image-tag-delta proofs. The gates do NOT prove the platform always recovers in-place from a Failed deployment — a different failure class (e.g., quota exhaustion mid-recovery) could force a tear-down-and-re-provision pattern. The full per-file provenance and honest-disclosure notes are in [`labs/acr-pull-failure/evidence/README.md`](https://github.com/yeongseon/azure-container-apps-practical-guide/blob/main/labs/acr-pull-failure/evidence/README.md).
