...
### Verify revision state in Azure Portal

![ca-sample-d38538 | Revisions and replicas | Container App | Create new revision | Save | Refresh | Deployment mode | Send us your feedback | Active revisions | Inactive revisions | Replicas | Name | Date created | Running status | View Logs | Label | Traffic | Replicas | ca-sample-d38538--0uzoi59 | Running | View details | Show Logs | 100 % | 1 (Show replicas)](../assets/best-practices/revision-strategy-revisions-and-replicas.png)

**[Observed]** `ca-sample-d38538 | Revisions and replicas` `Container App` `Create new revision` `Save` `Refresh` `Deployment mode` `Send us your feedback` `Each revision is an immutable snapshot of your container app, and can have different setups for traffic allocation, container images, autoscaling, or Dapr.` `Active revisions` `Inactive revisions` `Replicas` `Name` `Date created` `Running status` `View Logs` `Label` `Traffic` `Replicas` `ca-sample-d38538--0uzoi59` `6/3/2026, 10:34:26 PM` `Running` `View details` `Show Logs` `100` `%` `1 (Show replicas)` `Overview` `Activity log` `Access control (IAM)` `Tags` `Application` `Containers` `Scale` `Volumes` `Settings` `Networking` `Ingress` `Custom domains` `CORS` `Security` `Monitoring` `Log stream` `Logs` `Console` `Alerts` `Metrics`.
