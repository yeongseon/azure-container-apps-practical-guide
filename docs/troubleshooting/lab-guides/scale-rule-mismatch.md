...
content_validation:
  ...
  lab_validation:
    ...
    notes: 'Full reproducible evidence pack captured 2026-06-22 in koreacentral (Azure CLI 2.83.0, containerapp ext 1.3.0b4). H1 gate scale_rule_mismatch_replicas_capped (replicas 1/1/1/1/1 under 60-concurrent / 90 s load, 0 scale events) and H2 gate scale_rule_fixed_replicas_scaled_events_observed (replicas 1/5/5/7/7, 19 scale events) both PASS. Earlier 2026-05-01 reproduction (CLI 2.70.0) and 2026-06-02 Portal captures (rg-aca-lab-scale-cap) are retained below as supplementary evidence.'
  core_claims:
...
| 6 | Update scale rule to `concurrentRequests=10` and `maxReplicas=10` | New revision created | New revision `--0000002` Healthy after 23 s | Pass |
| 7 | Run `verify.sh` | Replica count increases under load | Replicas at +15 s/+30 s/+60 s/+90 s = 5/5/7/7 (max 7), 19 scale events in `ContainerAppSystemLogs_CL` | Pass (H2 gate `scale_rule_fixed_replicas_scaled_events_observed`) |
...
**Reproduction Azure CLI:** 2.83.0, containerapp extension 1.3.0b4.
...
{
  "load_window": {
    "start_utc": "2026-06-22T14:29:23Z",
    "end_utc": "2026-06-22T14:31:14Z",
    "duration_seconds": 90,
...
[Observed] H1 (pre-fix) — `ContainerAppSystemLogs_CL` returned zero rows for the strict load window `[2026-06-22T14:29:23Z, 2026-06-22T14:31:14Z]` filtered to `RevisionName_s == "ca-labscale-q2xnsr--0000001"` AND `Reason_s startswith "Scal"`. Captured to `labs/scale-rule-mismatch/evidence/09-system-logs-scale-events-pre-fix.json` as the literal empty JSON array `[]`.
...
```json
{
  ...
  "scale_events_count_after_fix": 19,
  "h2_all_subgates_pass": true,
  "gate_classification": "scale_rule_fixed_replicas_scaled_events_observed"
}
```

[Observed] H2 (post-fix) — `ContainerAppSystemLogs_CL` returned 19 rows for the strict post-fix load window `[2026-06-22T14:32:47Z, 2026-06-22T14:34:52Z]` filtered to `RevisionName_s == "ca-labscale-q2xnsr--0000002"` AND `Reason_s startswith "Scal"`. Captured to `labs/scale-rule-mismatch/evidence/18-system-logs-scale-events-after-fix.json`. Sample rows show `Reason_s=AssigningReplica`, `Log_s=Replica 'ca-labscale-q2xnsr--0000002-6f84dd5bc7-rqjf6' has been scheduled to run on a node.`

[Inferred] H2 — holding the load generator, container image, ingress, and FQDN constant, and changing only `concurrentRequests` (500 → 10) and `maxReplicas` (2 → 10), produced horizontal scaling up to 7 replicas with 19 KEDA-attributed scale events. The reproducible falsification gate `scale_rule_fixed_replicas_scaled_events_observed` is SUPPORTED. The alternative theories considered in H1 that were global to the workload — "load not real," "ingress/load path not reaching the app," or "the platform cannot scale this workload at all" — are falsified: the same load against the same FQDN, with only the scale rule changed, now drives horizontal scale-out with KEDA attribution rows in the system log table.

[Inferred] H2 — reaching 7 replicas (not all 10) under this specific load shape does not by itself prove `concurrentRequests=10` is the *optimal* threshold; it only proves the threshold and cap together are now permissive enough to scale meaningfully under 60-concurrent load. Tuning the threshold for production traffic requires correlating concurrent in-flight requests against latency and replica cost over a representative window.
