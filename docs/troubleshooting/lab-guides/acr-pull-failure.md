...
| Command | Purpose |
|---|---|
| `export APP_NAME=$(az containerapp list --subscription "$AZ_SUBSCRIPTION" --resource-group "$RG" --query "[0].name" --output tsv)` | Captures the only Container App name in the dedicated lab resource group so the rest of the evidence-collection commands target the failed app even though deployment outputs are null. |
