...
[Inferred] The presence of per-profile `Up to 4 vCPUs / 8 Gi`, `Current cores usage`, `Current instances`, and `Min & Max number of instances` columns at this blade is consistent with this page's [Scope stack](#scope-stack) diagram placing capacity governance at the environment scope rather than the application or revision scope. The dashed values in the usage and instance columns are consistent with this page's [Soft vs. hard boundaries](#soft-vs-hard-boundaries) framing that runtime usage and configured limits are tracked separately from fixed platform limits.
