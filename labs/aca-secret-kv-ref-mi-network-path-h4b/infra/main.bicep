...
// -----------------------------------------------------------------------------
// aca-secret-kv-ref-mi-network-path-h4b — infrastructure
// -----------------------------------------------------------------------------
//
...
//
// H4b is the logging-gap inversion of H4a.
//
// Controlled variables by phase:
//   H0 baseline: Entra Application Rule PRESENT + AzureFirewallApplicationRule
//                diagnostic category ENABLED -> `secret set` succeeds.
//   H1 trap:     Entra Application Rule REMOVED + diagnostic category DISABLED
//                -> `secret set` fails, but the firewall denial is not logged.
//   H2 fix:      Entra Application Rule STILL REMOVED + diagnostic category
//                RE-ENABLED -> `secret set` STILL fails, but the denial becomes
//                visible in Log Analytics after a NEW attempt.
//
// Connectivity does NOT recover in H2. H2 restores observability only.
//
// One named Application Rule allows BOTH
//     login.microsoftonline.com
//     login.microsoft.com
// H0: rule present                      -> `secret set` succeeds.
// H1/H2: rule absent                    -> `secret set` fails with EOF.
//
  ...
  properties: {
    ...
    ruleCollections: [
      ...
      }
      {
        ...
        // ENTIRE rule collection for H1 and keeps it absent for H2.
        // H2 restores observability by re-enabling diagnostics, not by
        // restoring connectivity. Keeping the two FQDNs together in a
        // single rule guarantees H1 is one atomic remove — no partial
        // removal that could accidentally leave one of the two hosts reachable.
        //
...
// appears in LAW, making the H4b logging-gap evidence impossible to capture.
//
...
// Firewall Application Rule collection name that H1 removes and H2 leaves absent.
