...
content_validation:
  ...
  lab_validation:
    ...
    notes: |
      ...
      All three falsification gates (baseline visibility, bypass silence,
      recover visibility) PASSED. This is the fifth and final lab in the
      5-lab ACR network path series, and it is the only lab in the series
      whose failure mode is "successful pull, silent inspection" rather
      than a visibly failed revision. The bypass-window pull of v-bypass
      reached Healthy in under 60 seconds with `/` returning
      `build_tag=v-bypass`, while `AZFWApplicationRule` recorded zero
      new rows for the ACR FQDN since the bypass-deploy timestamp. After
      re-adding the /32 UDR routes for both PE NIC IPs (10.90.2.4 and
      10.90.2.5) and deploying v-recover, the recover-gate query
      returned 10 new `AZFWApplicationRule` rows for the ACR FQDN
      within ~70 seconds of the recover-deploy timestamp.

      Two CRITICAL Bicep fixes (vs. v0) that this lab required:

      (1) `logAnalyticsDestinationType: 'Dedicated'` is REQUIRED on the
      firewall's `Microsoft.Insights/diagnosticSettings` resource.
      Without it, firewall diagnostics default to the legacy
      `AzureDiagnostics` schema and the `AZFWApplicationRule`
      resource-specific table stays empty forever. The original Bicep
      omitted this property, which is why the first falsify.sh run
      had no smoking-gun rows to assert against. Adding it at line ~407
      (`logAnalyticsDestinationType: 'Dedicated'`) flipped the schema
      and unlocked the resource-specific tables.

      (2) `falsify.sh` queries BOTH schemas via `union isfuzzy=true`.
      Even after patching live diagnostic settings to Dedicated, the
      `AZFWApplicationRule` table took ~10-20 minutes to backfill while
      the legacy `AzureDiagnostics` table kept receiving rows. The
      schema-tolerant query (extracting FQDN from `msg_s` via regex for
      AzureDiagnostics, reading the structured `Fqdn` column for
      AZFWApplicationRule) ensures the gate works regardless of which
      schema is active at query time. The baseline-visibility gate fired
      from AzureDiagnostics rows on the live run; the recover gate fired
      from AZFWApplicationRule Dedicated rows.

      Mirrors Lab 1's admin-creds pattern: the Bicep was refactored from
      managed-identity to admin credentials (`adminUserEnabled=true`)
      and `trigger.sh` wires the credentials via
      `az containerapp registry set --username --password`. This removes
