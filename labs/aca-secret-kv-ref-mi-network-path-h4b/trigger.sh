...
DEPLOYMENT_NAME="aca-secret-kv-ref-mi-network-path-h4b"
...
FIREWALL_RESOURCE_ID="$(echo "$OUTPUTS_JSON" | jq -r .firewallResourceId.value)"
...
cat > "$EVIDENCE_DIR/01-deployment-outputs.json" <<EOF
{
  ...
  "lab_name": "aca-secret-kv-ref-mi-network-path-h4b",
  "resource_group": "$RG",
  "location": "$LOCATION",
  "base_name": "$BASE_NAME",
  "app_name": "$APP_NAME",
...
if [[ "$KV_READY" != "yes" ]]; then
    KV_SCOPE_ID_FOR_DEBUG="$(az keyvault show --name "$KV_NAME" --resource-group "$RG" --query id --output tsv 2>/dev/null || true)"
    echo "ERROR: KV data-plane never became reachable via signed-in user identity after 5 minutes."
    echo "  Check: 'az role assignment list --scope $KV_SCOPE_ID_FOR_DEBUG --assignee $DEPLOYMENT_PRINCIPAL_ID'"
    exit 1
fi
