...
1. The image-pull HTTPS connection from the Container Apps workload subnet to `acracrfwallowt2itdu.azurecr.io` was forced through Azure Firewall by the `0.0.0.0/0` UDR on `snet-aca`. (The Container Apps workload subnet hosts both user replicas and platform-managed components, including the component that performs image pulls; Container Apps does not expose which specific component owns a given source IP, so this lab uses "Container Apps workload-subnet source IP" rather than "replica IP" for the pull-path source.)
