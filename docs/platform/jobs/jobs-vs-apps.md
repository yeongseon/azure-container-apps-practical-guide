...
![cj-sample-d38538 Container App Job showing Trigger Type Manual, Execution history View link, and Parallelism and Completion count fields](../../assets/platform/jobs/01-job-overview.png)
...
**[Inferred]** The resource subtype label `Container App Job` appears to map to the `Container Apps Job` column of the [Decision matrix](#decision-matrix), which on this page is presented separately from the `Container App` column. The `Trigger Type` field appears to correspond to the `Trigger` row of the [Decision matrix](#decision-matrix), and the `Execution history` field is consistent with the `Operational artifact` row entry `Execution history`. The `Parallelism` and `Completion count` fields are consistent with the [When Jobs are the better choice](#when-jobs-are-the-better-choice) framing that Jobs cleanly start, process, and exit.
