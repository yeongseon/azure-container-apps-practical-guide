...
content_validation:
  ...
  - claim: This page uses Microsoft Learn as the primary source basis for its Azure-specific
      guidance.
    source: https://learn.microsoft.com/azure/templates/microsoft.app/containerapps
    verified: true
---
...
**[Not Proven]** The Bicep template body referenced by the `Microsoft.App/containerApps@2024-03-01` resource declaration shown in the [Example Bicep snippet (.NET App with Health Probes)](#example-bicep-snippet-net-app-with-health-probes) is not visible on this view. The `az deployment group create` command invocation shown in the Bicep deployment flow is not visible on this view. The `az containerapp create` command output containing the ingress FQDN from [Step 4: Create Container App with environment variables](#step-4-create-container-app-with-environment-variables) is not visible on this view. The per-resource provisioning history record for Bicep deployments is not visible on this view; the `Deployments` essentials field renders only `No deployments`.
