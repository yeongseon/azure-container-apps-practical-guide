...
**[Inferred]** The `Environment type` `Workload profiles` value is consistent with the v2-recommended environment surface discussed in [Migration considerations](#migration-considerations). The `Applications` `1` value paired with the single `ca-sample-d38538` `Container App` row is consistent with the per-replica execution model described in [What the Consumption-only environment is](#what-the-consumption-only-environment-is). The `Applications` `1` value also appears to map to the low-baseline workload patterns referenced in [Good use cases](#good-use-cases).
