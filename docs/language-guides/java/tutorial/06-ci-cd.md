...
**[Not Proven]** The `AZURE_CREDENTIALS` JSON fields from [Setting up GitHub Secrets](#setting-up-github-secrets) are not visible on this view. The `${{ secrets.ACR_NAME }}.azurecr.io/java-guide:${{ github.sha }}` image reference passed to `--image` in [Creating the Workflow File](#creating-the-workflow-file) is not visible on this view. The `az acr build` command and Maven `mvn package -DskipTests` build invoked by the GitHub Actions workflow in [Creating the Workflow File](#creating-the-workflow-file) are not visible on this view. The `<your-app-name>--xxxxxxx` revision name returned by the `az containerapp revision list` command in [Verifying the Deployment](#verifying-the-deployment) is not visible on this view.
