...
- **One sub-resource, multiple endpoints.** ACR's Private Endpoint exposes a single `registry` sub-resource, but the PE NIC holds separate private IPs for the global/login endpoint and for each region's dedicated data endpoint. The corresponding records (`<registry>` and `<registry>.<region>.data`) must both exist in `privatelink.azurecr.io` for Path B to actually be Path B end to end. Resolving only the login record privately is **Scenario D** (record-level split-brain), not Path B.
...
- **In Azure Container Apps, Scenario E may surface at the workload layer first.** A broken custom DNS forwarder can surface as a workload-path failure rather than as an `ImagePullBackOff`, because the platform-managed image puller can continue to succeed while application and runtime DNS follow the broken custom resolver path. The reproduction in the [ACR Network Path E lab](../../troubleshooting/lab-guides/acr-network-path-dns-forwarder-bypass.md) shows the revision staying `Healthy` with `PulledImage` events while `socket.getaddrinfo("<acr>.azurecr.io")` from inside the replica returns the public registry IP. Treat pull-path health as **not** a reliable early-warning signal for DNS-topology failures in Container Apps; instrument workload-side DNS resolution explicitly.
