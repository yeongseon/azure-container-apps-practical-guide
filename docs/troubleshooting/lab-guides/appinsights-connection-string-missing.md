...
az monitor app-insights query \
    ...
    --app "$APP_INSIGHTS_NAME" \
    --resource-group "$RG" \
    --analytics-query "requests | where timestamp > ago(15m) | project timestamp, name, resultCode, duration, url, success | order by timestamp asc | take 25" \
    --output json
...
| `00-verify-run.txt` | Full `verify.sh` execution log (20 HTTP 200 + AppRequests aggregate `requestCount=20` + AppTraces aggregate `traceCount=21`) |
