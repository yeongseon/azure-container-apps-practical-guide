...
**[Inferred]** The `Environment variables` tab appears to map to the `--set-env-vars` lever exercised in [Setting Environment Variables via CLI](#2-setting-environment-variables-via-cli). The `Image` field value `java-sample` and `Image tag` value `v1` appear consistent with the `--image` reference set by `az containerapp update --image` invoked across the configuration steps in [Setting Environment Variables via CLI](#2-setting-environment-variables-via-cli). The `Registry` field value `acrbasicsd38538.azurecr.io` is consistent with the ACR login server referenced by the same `--image` reference. The `Authentication` value `Managed identity` is consistent with the managed-identity configuration in [Configure Managed Identity](#2-configure-managed-identity).
