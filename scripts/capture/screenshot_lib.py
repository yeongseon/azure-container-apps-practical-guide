...
class Shot:
    def __init__(self, raw: ShotEntry) -> None:
        self._raw = raw
...
class Manifest:
    def __init__(self, path: Path = MANIFEST_PATH) -> None:
        ...
        self._index: dict[str, ShotEntry] = {}
        seen_files: dict[str, str] = {}
        entries = self._data.get("screenshots", [])
        if not isinstance(entries, list):
            raise ManifestError("screenshots must be a list")
        for raw_entry in entries:
            if not isinstance(raw_entry, dict):
                raise ManifestError("screenshot entries must be objects")
            entry: ShotEntry = raw_entry
            self._validate(entry, seen_files)
            self._index[entry["id"]] = entry
            seen_files[entry["file"]] = entry["id"]
        self._pending: dict[str, dict[str, str]] = {}

    def _validate(self, entry: ShotEntry, seen_files: dict[str, str]) -> None:
    ...
    def save(self) -> None:
        ...
        entries = doc.get("screenshots", [])
        if not isinstance(entries, list):
            raise ManifestError("screenshots must be a list")
        for entry in entries:
            if not isinstance(entry, dict):
                raise ManifestError("screenshot entries must be objects")
            changes = self._pending.get(str(entry["id"]))
            if changes:
