...
| Replica Node Spread | Empirical test of how Consumption vs Dedicated D8 workload profiles distribute replicas across underlying shared-kernel contexts using `boot_id`/`uptime` proxy signals; documents bimodal `[Strongly Suggested]` contrast (per-replica node identity remains `[Inferred]`). | Advanced | 1.5-2 hours | [Guide](./replica-node-spread.md) | [Directory](https://github.com/yeongseon/azure-container-apps-practical-guide/tree/main/labs/replica-node-spread) |
