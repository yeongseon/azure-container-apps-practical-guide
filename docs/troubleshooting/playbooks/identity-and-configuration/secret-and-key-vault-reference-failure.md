...
The base H4 hypothesis assumes a specific topology: hub-spoke networking + `0.0.0.0/0` UDR forcing egress through Azure Firewall Standard + missing Application Rule + AzFW diagnostic settings enabled + `Deny` row visible in `AZFWApplicationRule`. The reproducer lab reproduces this exact scenario end-to-end.
...
- Firewall diagnostic settings enable both `AZFWApplicationRule` (structured schema) AND `AzureDiagnostics` (legacy schema) categories at the Firewall resource — verify with `az monitor diagnostic-settings list --resource $FIREWALL_ID`.
...
| **H4c (NSG deny)** | Workload subnet NSG blocks outbound 443/tcp to Entra IP ranges before AzFW inspection | NSG denies happen before AzFW; AzFW never sees the packet | NSG flow logs on the workload subnet (`az network watcher flow-log show`) contain `Deny` records for outbound 443/tcp to IPs in Microsoft-published Entra service tag ranges | Add an outbound NSG rule permitting 443/tcp to the `AzureCloud` service tag (or the more specific tag the customer environment uses), then re-verify |
| **H4d (Virtual WAN + Routing Intent)** | Virtual WAN Hub + Azure Firewall + Routing Intent policy (`InternetTraffic + PrivateTraffic → AzFW`) instead of hub-spoke + UDR | Traffic path differs from hub-spoke UDR model. Routing Intent may route outbound through an unexpected next-hop (a different regional hub, or a bypass path), or a regional propagation delay after a policy edit may leave a stale route in place for several minutes. AzFW may never receive the packet if the effective route does not target it. | `az network vhub get-effective-routes --resource-type Subnet --resource-id $ACA_INFRA_SUBNET_ID --name $VHUB_NAME --resource-group $VWAN_RG` (verify next-hop targets the expected AzFW); `az network vhub routing-intent show --name defaultRoutingIntent --parent-name $VHUB_NAME --resource-group $VWAN_RG` (verify Routing Intent policy is enabled and targets AzFW for `InternetTraffic`) | Verify the Routing Intent policy targets the correct AzFW resource; wait for regional hub propagation (up to 15 minutes after policy edit); temporarily disable Routing Intent to isolate the variable; escalate to Azure Support for Virtual WAN-scope routing behavior investigation |
| **H4e (Private DNS override)** | A Private DNS zone (`privatelink.microsoftonline.com` or a custom zone) is linked to the workload VNet and resolves the Entra authority hosts to private IPs that do not route | TCP connection times out at DNS resolution or at TCP handshake; nothing reaches the firewall | `az containerapp exec --name $APP_NAME --resource-group $RG --command "nslookup login.microsoftonline.com"` returns a private IP address (RFC1918); OR `az network private-dns zone list --resource-group $RG` shows a zone matching an Entra authority FQDN linked to the workload VNet | Remove the Private DNS zone virtual-network link, delete the overriding A records, or configure conditional forwarders that resolve Entra authority hosts against Azure public DNS |
...
| **H4g (TLS inspection with untrusted intermediary cert)** | AzFW Premium or 3rd-party NVA performs TLS inspection on outbound HTTPS, but the MI client's base container image does not trust the inspection root CA | The block is at the TLS handshake layer, not at the domain layer. AzFW logs may still show an `AZFWApplicationRule` row for the Entra authority FQDN but with a TLS handshake error signal rather than a plain `Deny`. | Base H4 KQL without the `Action == "Deny"` filter, looking for `AZFWApplicationRule` rows with TLS-related error columns or `msg_s` fields containing `tls`, `handshake`, `certificate`, or `SNI`; alternatively, `az containerapp exec` running `openssl s_client -connect login.microsoftonline.com:443 -showcerts` from a replica shows an unexpected intermediary certificate chain | Deploy the TLS inspection root CA certificate into the Container App's base image trust store (rebuild the image with the CA added to `/etc/ssl/certs`), OR configure the AzFW Premium / NVA TLS inspection policy to bypass Entra authority FQDNs |

| Command | Why it is used |
|---|---|
| `az monitor diagnostic-settings list --resource $FIREWALL_ID` | Confirms whether `AzureFirewallApplicationRule` and `AzureFirewallNetworkRule` diagnostic categories are enabled at the Firewall resource. If the category is missing or disabled, KQL against `AZFWApplicationRule` will return zero rows even when the firewall is actively dropping packets (H4b). |
| `az network vhub get-effective-routes --resource-type Subnet --resource-id $ACA_INFRA_SUBNET_ID --name $VHUB_NAME --resource-group $VWAN_RG` | Retrieves the effective route table for the ACA infrastructure subnet from the Virtual WAN Hub's perspective, so the actual next-hop for `0.0.0.0/0` can be confirmed. In H4d, this may reveal the traffic is routed to an unexpected hub or bypass path rather than to the expected AzFW private IP. |
| `az network vhub routing-intent show --name defaultRoutingIntent --parent-name $VHUB_NAME --resource-group $VWAN_RG` | Reads the Routing Intent policy on the Virtual WAN Hub so the `InternetTraffic` and `PrivateTraffic` next-hop targets can be verified. If Routing Intent is enabled but the policy does not target the expected AzFW, or if a recent edit has not fully propagated across regional hubs, egress may bypass firewall inspection entirely (H4d). |
...
| `az network private-dns zone list --resource-group $RG` | Enumerates Private DNS zones in the resource group so any zone matching an Entra authority FQDN linked to the workload VNet can be identified. A `privatelink.microsoftonline.com` zone (or similarly-named custom zone) linked to the workload VNet is the direct H4e trigger. |
| `az network watcher flow-log show` | Reads NSG flow log configuration for the workload subnet NSG. If flow logs are enabled and contain `Deny` records for outbound 443/tcp to IPs in Microsoft-published Entra service tag ranges, the block is at the NSG layer before AzFW inspection (H4c). |
| `openssl s_client -connect login.microsoftonline.com:443 -showcerts` (run via `az containerapp exec`) | Establishes a raw TLS connection to the Entra authority from inside a workload replica and prints the full certificate chain the client actually receives. If the leaf certificate is issued by an unexpected intermediary (a corporate TLS inspection CA rather than a public Microsoft-issued chain), the MI client's TLS handshake will fail with a certificate verification error even though the domain reaches the firewall (H4g). |
...
- **"Virtual WAN Hub + Firewall + Routing Intent" + "customer reports no firewall block logs"** → most commonly **H4d combined with H4b**. Verify Routing Intent policy state and Firewall diagnostic settings in parallel. If Routing Intent effective routes look correct AND diagnostic settings are enabled AND KQL still returns zero rows, the failure may be a regional hub propagation delay; wait 15 minutes and re-check.
- **"3rd-party NVA in path" + "no AzFW logs"** → **H4f**. Engage the NVA vendor with the failure timestamp and destination FQDN list.
- **"AzFW logs show TLS handshake errors on Entra authority FQDN"** → **H4g**. Deploy the inspection root CA to the workload image or exempt Entra authority FQDNs from TLS inspection.
...
6. If Step 5 confirms the firewall Application Rule for both Entra authority FQDNs is already present AND the base H4 KQL returns zero rows in the failure window, work through the [H4 Variants table](#h4-variants-when-base-h4-kql-returns-zero-rows) in rule-out order. Cheapest verification first: check AzFW diagnostic settings enabled at the Firewall resource (H4b, one CLI call), then verify DNS resolution of the Entra authority hosts from the workload path (H4e, one `az containerapp exec` call for `nslookup`), then inspect NSG rules on the workload subnet (H4c), then verify Virtual WAN Hub routing intent and effective routes if the customer environment uses Virtual WAN (H4d), then check TLS inspection certificate trust if AzFW Premium or an NVA is doing TLS inspection (H4g), and finally 3rd-party NVA policy if the customer environment has a vendor NVA in path (H4f). The customer scenario "Virtual WAN Hub + Firewall + Routing Intent + no firewall block logs" typically resolves as H4d combined with H4b — verify Routing Intent state and Firewall diagnostic settings in parallel, and account for regional hub propagation delays (up to 15 minutes after a policy edit).
