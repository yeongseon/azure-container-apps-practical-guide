...
| Command | Purpose |
|---|---|
| `az extension add --name containerapp --upgrade` | Ensures the Container Apps CLI extension is installed and current before you run any Node.js recipe commands. |
| `az login` | Starts an authenticated Azure CLI session so the recipe commands can read and update Azure resources. |
