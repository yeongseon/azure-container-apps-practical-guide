...
`verify.sh` is a pure file processor against the 35 historical-capture artifacts under `evidence/ca-oom-{hard,leak,healthy}/` and emits four falsifiable gate JSONs under `evidence/`. Each gate is evaluated against a **strict 2-path predicate** per sub-gate — a **Strong path** that matches the exact lab specification (e.g. record-scoped 2-form OOM predicate matching the platform's two distinct exit-event row shapes, or §7 leak-tick regex parse) and a **Fallback path** that tolerates the same controlling behavior under minor numeric drift (e.g. the §6 5-minute bin histogram as a weaker proxy for the §5 record count). Eleven of the twelve sub-gates (4 gates × 3 sub-gates each) emitted on 2026-06-24 pass on **both** the Strong and Fallback paths against the canonical 2026-06-20 evidence. The twelfth — Gate 12 sub-gate (a) — passes on the Strong path only; its Fallback path is **intentionally `false`** by design (see the detailed disclosure two paragraphs below). All twelve `*_pass` flags evaluate to `true` because each sub-gate composes as `Strong OR Fallback` and the Strong path alone is sufficient.

| Gate | File | Hypothesis | Sub-gates | Result |
|---|---|---|---|---|
| 10 | `10-h1-hard-oom-immediate-gate.json` | H1.a (Scenario A hard-oom): a workload that allocates beyond the cgroup memory ceiling at startup produces an immediate, dense OOM burst signature distinct from a gradual leak | 3 — (a) signal observed: §5 has ≥ 10 OOM records (record-scoped 2-form predicate, see below) OR §6 max 5-min bin count ≥ 10; (b) hard signature: §6 max 5-min bin ≥ 10 (dense burst) OR §6 bin count ≥ 3 (multi-bin sustained pattern); (c) state eventually fixed: sidecar `healthState == "Healthy"` (Strong) OR sidecar `provisioningState != "Failed"` (Fallback) | PASS — 16 §5 OOM records (11 form-A + 5 form-B), §6 max bin 26 events @ 04:10 UTC, §6 has 23 bins spanning 04:10–06:00 UTC, sidecar `healthState: Healthy` + `provisioningState: Provisioned` + `runningState: RunningAtMaxScale` + `trafficWeight: 0` (POST-fix revision) |
| 11 | `11-h1-leak-delayed-oom-gate.json` | H1.b (Scenario B leak): a workload that allocates +30 MiB / 20 s in a background thread produces a delayed OOM signature with a measurable memory runway (monotonic tick prints) | 3 — (a) leak runway observed: §7 parses ≥ 12 leak-tick records via regex `\[leak\] tick (\d+): \+30 MiB, total retained (\d+) MiB` OR ≥ 8 ticks (fallback); (b) tick + retained monotonic: tick numbers strictly increasing AND retained MiB strictly increasing OR retained values fit arithmetic invariant `retained[i] = 30 × tick[i]`; (c) leak eventually kills: §5 has ≥ 1 OOM record OR §6 has any nonzero bin | PASS — 16 §7 ticks parsed (1→16, retained 30→480 MiB, strict monotonic both ways, arithmetic invariant holds), 1 §5 OOM record (form-A), §6 has 2 bins (13/3 at 04:10/04:15) |
| 12 | `12-h2-healthy-control-gate.json` | H2 (Scenario C healthy baseline): a workload with no allocations produces zero OOM records, falsifying any "OOMs are environmental" theory | 3 — (a) no OOM records: §5 has 0 OOM records (Strong) — Fallback intentionally `false` because §6 `has_any('memory')` KQL filter produces false-positives on the healthy scenario; (b) healthy state: sidecar `healthState: Healthy` OR sidecar `provisioningState: Provisioned`; (c) stable replicas: sidecar `runningState: RunningAtMaxScale` AND `replicas ≥ 1` (substitutes for absent §10c restart-count metric) OR sidecar `trafficWeight: 100` | PASS — 0 §5 OOM records, sidecar `healthState: Healthy` + `provisioningState: Provisioned` + `runningState: RunningAtMaxScale` + `replicas: 1` + `trafficWeight: 100` |
| 13 | `13-h3-cross-scenario-falsification-gate.json` | H3 (cross-scenario): three workload patterns produce three distinguishable OOM signatures — hard dense burst vs leak monotonic runway vs healthy silence | 3 — (a) OOM count ordering: exact `healthy=0 AND leak ≥ 1 AND hard ≥ 10` OR weak `healthy < leak < hard`; (b) distinct signature classes: hard `§6 max bin ≥ 10` AND leak `§7 tick count ≥ 12` AND healthy `§5 OOM = 0` OR weak `hard has dense bin AND leak has ticks AND healthy has no OOM`; (c) health states match outcome: `healthy.healthState == "Healthy" AND leak.healthState ∈ {Healthy, Unhealthy} AND hard.healthState ∈ {Healthy, Unhealthy}` (Strong — the control must be Healthy; the other two scenarios' latest-sidecar state is tolerated as Healthy OR Unhealthy because their snapshot timing relative to the platform's restart cadence is variable, see honest disclosure) OR `healthy.healthState == "Healthy"` (Fallback — control-only, used when the failing scenarios' snapshots fall in an unexpected rolling-deploy state) | PASS — OOM counts 0 / 1 / 16, signature classes hard `max bin 26 ≥ 10` + leak `16 ticks ≥ 12` + healthy `0 OOM`, all three latest sidecars `Healthy` (which satisfies both the Strong and Fallback paths because all three observed states are within the allowed sets) |

The Gate 10 sub-gate (a) Strong path uses a **record-scoped 2-form OOM predicate** rather than a whole-file substring search. The Container Apps platform emits two distinct exit-event row shapes for the same OOM event: form A `Reason_s == ContainerTerminated AND Log_s contains 'exit code \'137\'' AND Log_s contains 'ProcessExited'` (11 records in hard) and form B `Reason_s == ProcessExited AND Log_s contains 'exit code \'137\''` (5 records in hard). The gate matches both forms and sums them — a whole-file substring search would inflate the count by counting the same OOM event in both rows. This per-record discipline is why Gate 10's `predicate_inputs` lists the report path (parsed) rather than asserting against the raw text.

The Gate 12 sub-gate (a) Fallback path is **intentionally `false` and documented as such in the gate JSON's `section_6_bins_note`**: the §6 5-minute bin histogram uses a KQL `has_any('memory')` filter that produces false-positives on the healthy scenario (14 events in the 04:10 bin that are platform "memory" mentions unrelated to OOM). Only the Strong path (§5 record-scoped 2-form predicate, which returned 0) is authoritative for Gate 12. This is intentional defensive design — letting the fallback path silently mask a regression would defeat the purpose of the gate. The Gate 13 sub-gate (b) Strong path keys off §7 leak-tick count (not §6 bins) for the leak signature for the same reason. See [`labs/memory-leak-oomkilled/evidence/README.md`](https://github.com/yeongseon/azure-container-apps-practical-guide/blob/main/labs/memory-leak-oomkilled/evidence/README.md) "Honest disclosure" section for the full empirical platform behavior documented during the 2026-06-20 live run, including the three-timestamps-for-hard pattern (two PRE-fix at 04:19:00Z + 04:21:13Z plus one POST-fix at 06:02:57Z), the leak scenario's 1-OOM cross-cycle overlap, and the §10c PTY-failure-on-macOS that motivated using `runningState: RunningAtMaxScale` as the Gate 12 sub-gate (c) substitute for restart count.

The H1.a gate (10) proves hard-OOM has an immediate dense burst signature. The H1.b gate (11) proves leak has a delayed monotonic-runway signature distinct from hard. The H2 gate (12) proves healthy has zero OOM, falsifying any environmental-cause theory. The H3 cross-scenario gate (13) proves the three signatures are mutually distinguishable. Together these four gates yield the operator guidance in the **Operator takeaway from experiment** subsection above: exit code 137 alone does not classify the failure shape; the §6 bin density, §7 tick presence, and §5 record count together distinguish hard-OOM from gradual-leak from non-OOM-baseline.
