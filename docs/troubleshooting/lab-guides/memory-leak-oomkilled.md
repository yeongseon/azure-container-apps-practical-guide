...
| 13 | `13-h3-cross-scenario-falsification-gate.json` | H3 (cross-scenario): three workload patterns produce three distinguishable OOM signatures — hard dense burst vs leak monotonic runway vs healthy silence | 3 — (a) OOM count ordering: exact `healthy=0 AND leak ≥ 1 AND hard ≥ 10` OR weak `healthy < leak < hard`; (b) distinct signature classes: hard `§6 max bin ≥ 10` AND leak `§7 tick count ≥ 12` AND healthy `§5 OOM = 0` OR weak `hard has dense bin AND leak has ticks AND healthy has no OOM`; (c) health states match outcome: exact `healthy=Healthy AND leak=Healthy AND hard=Healthy` (all three latest sidecars Healthy by design — see honest disclosure) OR weak `all states present` | PASS — OOM counts 0 / 1 / 16, signature classes hard `max bin 26 ≥ 10` + leak `16 ticks ≥ 12` + healthy `0 OOM`, all three latest sidecars `Healthy` |
