...
`labs/` 폴더에는 실제 Container Apps 이슈를 재현하는 Bicep 템플릿과 함께 실습 실험이 포함되어 있습니다. 각 실험의 구성은 다음과 같습니다:
...
- **경로 B — PE 직접 연결** — `privatelink.azurecr.io` 연결된 영역이 있는 ACR Premium 사설 엔드포인트
- **경로 C — PE 강제 검사** — 사설 엔드포인트 + Azure Firewall + `/32` UDR 경로 (조용한 검사 우회 클래스)
