...
result = {
    ...
    "cohort_binding_note": (
        "Single-variable claim is bounded to the (cpu, memory) delta on the "
        "shared-fields surface of 01 and 06. The cohort does NOT prove: "
        "(1) that the underlying Kubernetes pod was reused -- lineage in "
        "sub-gate (b) explicitly shows a NEW revision was created and the "
        "old revision is now in runningState==Deprovisioning; "
        "(2) that the container image is byte-identical -- this is inferred "
        "from the fact that the Phase A fix-and-capture.sh issued "
        "`az containerapp update --cpu --memory` with no `--image`, "
        "`--command`, or `--args` flag, but the cohort does not capture image "
        "digests so this inference is a documented gap in the claim ceiling. "
        "(3) The activeRevisionsMode field appears only in 01 and the "
        "latestRevisionName field appears only in 06; this is a known "
        "asymmetry in the `az containerapp show` query projection used in "
        "Phase A and is intentionally excluded from the diff set."
    ),
    "gate_classification": (
