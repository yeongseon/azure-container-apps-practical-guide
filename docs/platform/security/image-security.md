...
| Azure Container Registry with managed identity | RBAC + managed identity | Strongest documented native Azure Container Apps option | Preferred production default |
...
Container Apps documentation describes **how to configure registries for an app**, but it does **not** document a native Container Apps feature called allowed registries or trusted registries that enforces image source allow-listing by itself.
...
!!! warning "Do not describe allowed registries as a native Container Apps enforcement feature"
    Microsoft Learn documents registry configuration and governance options around Azure resources, but not a built-in allow-list switch specific to Container Apps for image sources. Treat allow-listing as a policy and supply-chain governance control.
...
| Control | Native Container Apps surface | ACR / governance surface |
|---|---|---|
...
| Source allow-listing | No documented native Container Apps allow-list feature | Policy / governance pattern |
