...
**[Inferred]** The `Environment type` `Workload profiles` value is consistent with the v2-recommended environment surface discussed in [Migration considerations](#migration-considerations). The `KEDA version` `2.18.1` value appears to map to the autoscaling-capability listing in [Characteristics and limitations](#characteristics-and-limitations). The `Dapr version` `1.16.4-msft.7` value is consistent with the platform-feature surface listed in [Characteristics and limitations](#characteristics-and-limitations). The `Applications` `1` value paired with the single `ca-sample-d38538` row is consistent with the small-app workload pattern referenced in [Good use cases](#good-use-cases).
