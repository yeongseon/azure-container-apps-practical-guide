#!/usr/bin/env bash
# capture-window.sh — Phase A live-Azure observation-window capture for the
...
#
# The LOG_FILE name 00-verify-run.txt is intentionally preserved for
# canonical-evidence schema stability — the committed Phase A evidence
# pack references this filename, and renaming it would orphan the
# canonical record. Future re-runs of capture-window.sh will overwrite
# 00-verify-run.txt at the same canonical path.
set -euo pipefail
...
echo "===== capture-window.sh started $(date -u +%Y-%m-%dT%H:%M:%SZ) ====="
...
echo "===== capture-window.sh evaluation ====="
