...
This guide provides a comprehensive reference implementation for running .NET applications on Azure Container Apps. We use a production-ready ASP.NET Core application to demonstrate best practices for cloud-native deployment, security, and observability on the Azure platform.

## Reference Application

The reference .NET application is located in the `apps/dotnet-aspnetcore/` directory. It is a production-hardened ASP.NET Core implementation designed to showcase modern cloud-native patterns.

Key features demonstrated in the reference app:

- **Health Probes**: Implements `/health` and `/info` endpoints to enable platform-managed lifecycle and zero-downtime deployments using ASP.NET Core middleware.
- **Structured Logging**: Native .NET logging configured for seamless integration with Azure Log Analytics and Application Insights.
- **OpenTelemetry**: Native support for distributed tracing and performance monitoring through the `Azure.Monitor.OpenTelemetry.AspNetCore` SDK.
- **Multi-stage Dockerfile**: Optimized build process using Alpine-based images for smaller attack surface and faster deployment.
...
- **.NET 8.0 SDK or higher**: Required for local development, testing, and dependency management.
...
Follow these step-by-step guides to master the deployment of .NET applications on Azure Container Apps:

1.  [**Local Development**](./tutorial/01-local-development.md) — Learn how to containerize and run your ASP.NET Core app in Docker on your local machine.
...
| Runtime tuning | Complete | [dotnet-runtime](./dotnet-runtime.md) |
...
For detailed technical information on how the .NET runtime is configured and optimized for Azure Container Apps, see the [.NET Runtime Reference](./dotnet-runtime.md).

This guide covers:
- ASP.NET Core Kestrel configuration and tuning.
- System-level and application-specific environment variable overrides.
- NuGet package management and Docker layer optimization strategies.
...
- Building production-grade Docker images optimized for the .NET ecosystem.
...
!!! note "Use standard variables consistently"
    For command consistency across tutorials and recipes, use `$RG`, `$APP_NAME`, `$ACA_ENV_NAME`, `$ACR_NAME`, and `$LOCATION` in your shell session before running commands.
