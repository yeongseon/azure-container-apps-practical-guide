...
| D | Split-DNS misconfiguration | PE private IP / **public IP** | Login over Private Link, image layers over public path | Yes for data, partial for login | Entra ID for MI | Never intentional — appears when the registry record (`<registry>`) exists in `privatelink.azurecr.io` but the per-region data record (`<registry>.<region>.data`) is missing or not resolving privately | `az acr login` from the replica looks fine, but actual layer downloads silently traverse the public path |
...
The same Container Apps environment can land on Path A, Path B, Path C, or Scenario D depending entirely on how DNS resolution is wired up.
...
| Custom DNS via Azure Private DNS Resolver | Replicas use the resolver inbound endpoint as recursive DNS; forwarder rules in the resolver decide whether queries hit Azure DNS or external DNS. | Misconfigured forwarder = ACR resolves public even though a PE exists. |
| Custom DNS via a customer-managed DNS server on a VM (for example, Windows DNS or AD-integrated DNS) | Replicas use the VM as recursive DNS; the VM must conditionally forward `privatelink.azurecr.io` to Azure DNS at `168.63.129.16` so that both the registry record and the per-region data record resolve via the Azure Private DNS zone. | Easy to forget that the forwarder covers the entire `privatelink.azurecr.io` zone (not just the registry record); a partial setup produces Scenario D. Single-VM DNS is also a reliability single point of failure. |
| Public resolvers (`8.8.8.8`, `1.1.1.1`, on-prem caching) | Returns ACR public IPs even when a Private Endpoint exists. | Forces Path A and silently bypasses the Private Endpoint. |
...
- **One sub-resource, multiple endpoints.** ACR's Private Endpoint exposes a single `registry` sub-resource, but the PE NIC holds separate private IPs for the global/login endpoint and for each region's dedicated data endpoint. The corresponding records (`<registry>` and `<registry>.<region>.data`) must both exist in `privatelink.azurecr.io` for Path B to actually be Path B end to end. Resolving only the login record privately is Scenario D, not Path B.
- **Geo-replication multiplies the data-endpoint surface.** Every replicated region needs its own `<registry>.<region>.data` record in `privatelink.azurecr.io`. Missing a region produces Scenario E.
