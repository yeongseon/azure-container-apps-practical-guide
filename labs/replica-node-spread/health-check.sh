#!/usr/bin/env bash
# health-check.sh — Phase A live-Azure readiness gate for the
...
# (no Azure calls) that emits the four gate JSONs under evidence/. The
# Phase A health-check role moved here so both scripts can coexist with
...
# Defensive guard: health-check is read-only but the per-check output
# would be confusing (all PASS or all FAIL) if it ran on the wrong
# subscription.
