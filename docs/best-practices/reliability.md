...
**[Inferred]** The `Running` value under `Running status` for revision `ca-sample-d38538--0uzoi59` is consistent with the probe-driven readiness model discussed in [Design health probes as separate reliability signals](#design-health-probes-as-separate-reliability-signals), which gates traffic on readiness probe outcomes. The `Active revisions` / `Inactive revisions` toggle alongside `Create new revision` appears to map to the keep-last-known-good rollback workflow described in [Revision-based rollback for speed and safety](#revision-based-rollback-for-speed-and-safety), which keeps a proven revision active while a new one is rolled out. The `Traffic` value `100 %` on a single active revision is consistent with the containment step in [Example incident workflow (15-minute containment target)](#example-incident-workflow-15-minute-containment-target), which routes traffic to the known-good revision during incident response. The `Replicas` value `1 (Show replicas)` appears to map to the minimum-replica decision in [Scale reliability: avoid thrash and cold-start shock](#scale-reliability-avoid-thrash-and-cold-start-shock), which calls out keeping non-zero minimum replicas for latency-sensitive APIs.

**[Not Proven]** Startup, readiness, and liveness probe definitions for this revision are not visible on this view. SIGTERM handling, termination grace period, and in-flight request drain behavior are not visible on this view. Zone-redundant placement of the replica across availability zones is not visible on this view. Dapr resiliency policies, retry budgets, and circuit-breaker state are not visible on this view.
