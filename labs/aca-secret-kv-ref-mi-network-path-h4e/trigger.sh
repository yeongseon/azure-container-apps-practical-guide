...
: "${RG:?RG must be set (e.g. rg-aca-kv-mi-netpath-h4e)}"
: "${LOCATION:?LOCATION must be set (e.g. koreacentral)}"
: "${BASE_NAME:?BASE_NAME must be set (3-11 chars, lowercase alphanumeric, e.g. acasech4e01)}"

DEPLOYMENT_NAME="aca-secret-kv-ref-mi-network-path-h4e"
...
cat > "$EVIDENCE_DIR/01-deployment-outputs.json" <<EOF
{
  ...
  "lab_name": "aca-secret-kv-ref-mi-network-path-h4e",
  "resource_group": "$RG",
  "location": "$LOCATION",
  "base_name": "$BASE_NAME",
  "app_name": "$APP_NAME",
...
cat > "$EVIDENCE_DIR/04-h0-secret-set-outcome.json" <<EOF
{
  "captured_at_utc": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "phase": "H0",
  "hypothesis": "Baseline: no custom DNS override for Entra authority -> az containerapp secret set SUCCEEDS",
